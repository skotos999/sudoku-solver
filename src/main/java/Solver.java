...
final class Solver {
	...
	private static Integer[] printSolution(final List<Node> solution) {
		...
		return lists.stream() //
				...
				.map(strings -> Integer.parseInt(String.valueOf(strings.get(0).charAt(2))))
				.toArray(Integer[]::new);
	}
	...
	static ColumnNode init(final Integer[] sudokuPuzzle) {
		...
		final var givenNumbers = matrix.stream()
				.filter(nodeList -> nodeList.stream()
						.anyMatch(Node::isGiven))
				.flatMap(List::stream)
	...
	private static void setHeadersSize(final List<ColumnNode> headers, final Map<String, List<Node>> nodes) {
		headers.stream().filter(header -> nodes.containsKey(header.getName())).forEach(header -> header.setSize(nodes.get(header.getName()).size()));
	}
